...
for x, start in enumerate(gals):
    for y, end in enumerate(gals[x+1:]):
        dist1 = sum(rows1[min(start[0],end[0]):max(start[0],end[0])]) + sum(cols1[min(start[1],end[1]):max(start[1],end[1])])
        sum1 += dist1
        dist2 = sum(rows2[min(start[0],end[0]):max(start[0],end[0])]) + sum(cols2[min(start[1],end[1]):max(start[1],end[1])])
        sum2 += dist2
